...
get(Key, WaitForRefresh) ->
    ...
    Res = case ets:lookup(?ETS_CACHE_TABLE, Key) of
        [#cache_entry{validity=Validity, value=Value}] when Now < Validity ->
            gen_server:cast(?MODULE, {set_stat, Key, hit}),
            {hit, Value};
        [#cache_entry{evict=Evict, value=Value, refresh_callback=undefined}] when Now < Evict ->
            gen_server:cast(?MODULE, {set_stat, Key, stale}),
            {stale, Value};
        ...
        [#cache_entry{value=_ExpiredValue}] ->
            ...
            evicted;
        [] ->
            gen_server:cast(?MODULE, {set_stat, Key, miss}),
            miss
    end,
    {ok, Res}.
...
handle_call({set, #cache_entry{key=Key}=Entry}, _From, #state{stats=StatsDict} = State) ->
    UpdatedStats = set_cache_entry(Key, Entry, StatsDict),
...
handle_cast({set_stat, Key, Stat}, #state{stats=Stats} = State) ->
    {noreply, State#state{
        stats=update_stats(Key, Stat, Stats)
    }};
handle_cast({set, #cache_entry{key=Key}=Entry}, #state{stats=StatsDict} = State) ->
    UpdatedStats = set_cache_entry(Key, Entry, StatsDict),
    {noreply, State#state{stats=UpdatedStats}};
handle_cast({evict, Key}, #state{stats=StatsDict} = State) ->
    ets:delete(?ETS_CACHE_TABLE, Key),
    {noreply, State#state{
        stats=update_stats(Key, evict, StatsDict)
    }};
...
-spec set_cache_entry(erl_cache:key(), dict(), #cache_entry{}) -> dict().
set_cache_entry(Key, Entry, Stats) ->
    ets:insert(?ETS_CACHE_TABLE, Entry),
    update_stats(Key, set, Stats).
...
-spec update_stats(erl_cache:key(), hit|miss|stale|evict, dict()) -> dict().
update_stats(Key, Stat, Stats) ->
    dict:update_counter({Key, Stat}, 1, Stats).
